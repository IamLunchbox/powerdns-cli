...
def mock_ctx():
    ...
    mock_ctx.obj.handler.get_result.return_value = {"success": True, "data": {"key": "value"}, "message": "Success"}
    return mock_ctx
...
def test_exit_cli_print_data(mock_ctx: click.Context, mocker) -> None:
    ...
    assert excinfo.value.code == 0
    click.echo.assert_called_once_with(json.dumps(mock_ctx.obj.handler.get_result()["data"], indent=4))
